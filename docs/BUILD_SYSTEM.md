...
make -j4
# Builds only: ria, cli_simulator, test tools
```
...
|--------|--------|--------------|---------|
| `ria` | main.cpp + modem_engine | ultra_core | CLI tool |
| `ria_gui` | gui/*.cpp + imgui | ultra_core, SDL2, OpenGL | GUI app |
...
├── ria                   # CLI tool
├── ria_gui               # GUI app (if SDL2+OpenGL)
