//! RIA - STANAG 4197-style 39-tone HF Modem
//!
//! A cross-platform amateur radio modem using STANAG 4197-style 39-tone DQPSK modulation,
...
use modem::{
    StanagModulator, StanagDemodulator, StanagConfig, Bandwidth as ModemBandwidth,
...
fn main() -> Result<()> {
    ...
    info!("RIA STANAG HF Modem v{}", env!("CARGO_PKG_VERSION"));
    info!("Starting up...");
    ...
    let window_title = if instance_id == "0" {
        format!("RIA - STANAG HF Modem [{}]", config.callsign)
    } else {
        format!("RIA - STANAG HF Modem [{}] (Instance {})", config.callsign, instance_id)
    };
...
fn run_modem_processing(
    ...
    let mut stanag_config = StanagConfig::for_speed_level(config.default_speed_level, modem_bw, sample_rate);

    let mut modulator = StanagModulator::new(
        StanagConfig::for_speed_level(config.default_speed_level, modem_bw, sample_rate),
        SpeedLevel::new_for_bandwidth(config.default_speed_level, config.bandwidth).constellation_for_bandwidth(config.bandwidth)
    );
    let mut demodulator = StanagDemodulator::new(
        StanagConfig::for_speed_level(config.default_speed_level, modem_bw, sample_rate),
        SpeedLevel::new_for_bandwidth(config.default_speed_level, config.bandwidth).constellation_for_bandwidth(config.bandwidth)
    );
    ...
    let stanag_symbol_period = stanag_config.fft_size; // Symbol period for autocorrelation
    ...
    loop {
        ...
        if speed_level != current_frame_config.level {
            ...
            // Recreate STANAG config, modulator, and demodulator for new mode
            ...
            stanag_config = StanagConfig::for_speed_level(speed_level, modem_bw, sample_rate);
            modulator = StanagModulator::new(
                StanagConfig::for_speed_level(speed_level, modem_bw, sample_rate),
                new_speed_level.constellation_for_bandwidth(config.bandwidth)
            );
            demodulator = StanagDemodulator::new(
                StanagConfig::for_speed_level(speed_level, modem_bw, sample_rate),
                new_speed_level.constellation_for_bandwidth(config.bandwidth)
            );
            info!("Recreated STANAG config/mod/demod for mode {} (FFT={}, carriers={})",
                speed_level, stanag_config.fft_size, stanag_config.num_carriers);
        ...
        if listen_enabled && !is_transmitting && !in_tx_cooldown && rx_buffer.len() >= stanag_config.fft_size + stanag_config.cp_length {
            ...
            if let Some(offset) = offset {
                ...
                // For STANAG: wait for enough symbols per frame config; for FSK: we check inside the FSK block
                let min_ofdm_samples = (stanag_config.fft_size + stanag_config.cp_length) * current_frame_config.symbols;
                ...
                if have_enough {
                    ...
                    let symbol_len = stanag_config.fft_size + stanag_config.cp_length;
                    ...
                    if data_rms < 0.01 {
                        ...
                        let cp_len = stanag_config.cp_length;
                        let fft_size = stanag_config.fft_size;
                        ...
                        debug!("CP timing: cp_len={}, fft_size={}, symbol_len={}, search_range={}", cp_len, fft_size, symbol_len, search_range);
                        ...
                        // Demodulate ALL STANAG symbols (not just one) starting from best offset
                        ...
                        let encoded_bits_expected = current_code_rate.ldpc_output_bits(block_size);
                        debug!("Demodulated {} soft bits from {} symbols, need {} for LDPC (block_size={}, rate {:?})",
                               all_soft_bits.len(), num_symbols, encoded_bits_expected, block_size, current_code_rate);
                        if all_soft_bits.len() >= encoded_bits_expected {
                            // Decode LDPC code
                            let decoded_bits = ldpc_decoder.decode(&all_soft_bits[..encoded_bits_expected]);
                            ...
                            debug!("LDPC decoded {} bytes", decoded_bytes.len());
                            ...
                            match Frame::decode(&decoded_bytes) {
                                ...
                                Err(e) => {
                                    ...
                                    pending_preamble_offset = None;
                                    pending_preamble_time = None;
                                    // Drain full estimated frame size (preamble + 48 STANAG symbols) to skip corrupted frame
                                    let preamble_len = preamble.samples().len();
                                    let stanag_frame_len = 48 * (stanag_config.fft_size + stanag_config.cp_length); // 48 STANAG symbols
                                    let full_frame_len = preamble_len + stanag_frame_len;
                                    let drain_len = full_frame_len.min(rx_buffer.len());
                                    rx_buffer.drain(0..drain_len);
                                    debug!("Drained {} samples after failed decode (preamble {} + STANAG {})", drain_len, preamble_len, stanag_frame_len);
                                    preamble_detector.reset();
                                }
                            }
                        }
