...
///
/// NOTE: Carrier counts here are the canonical source for STANAG configuration.
...
pub struct RateInfo {
    ...
    pub symbol_rate: f32,
    /// Number of carriers (2300 Hz mode) - canonical source for STANAG config
    pub carriers: usize,
    /// Number of carriers (500 Hz mode) - canonical source for STANAG config
    pub carriers_500: usize,
    /// Number of carriers (2750 Hz mode) - canonical source for STANAG config
...
pub struct FrameConfig {
    ...
    pub fft_size: usize,
    /// Number of STANAG symbols per frame
    pub symbols: usize,
...
/// Frame size for sliding frame adaptation
/// Frame size varies based on channel conditions to optimize throughput vs. reliability
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Default)]
pub enum FrameSize {
    /// 25% of base symbols - poor conditions, fast ACKs
    Short,
    /// 50% of base symbols - moderate conditions
    Medium,
    /// 100% of base symbols - normal operation
    #[default]
    Standard,
    /// 150% of base symbols - excellent conditions, higher throughput
    Long,
}

impl FrameSize {
    /// Get symbol count multiplier for this frame size
    pub fn multiplier(&self) -> f32 {
        match self {
            FrameSize::Short => 0.25,
            FrameSize::Medium => 0.50,
            FrameSize::Standard => 1.00,
            FrameSize::Long => 1.50,
        }
    }

    /// Encode frame size as 2 bits (0-3)
    pub fn to_bits(&self) -> u8 {
        match self {
            FrameSize::Short => 0,
            FrameSize::Medium => 1,
            FrameSize::Standard => 2,
            FrameSize::Long => 3,
        }
    }

    /// Decode frame size from 2 bits (0-3)
    pub fn from_bits(bits: u8) -> Self {
        match bits & 0x03 {
            0 => FrameSize::Short,
            1 => FrameSize::Medium,
            2 => FrameSize::Standard,
            _ => FrameSize::Long,
        }
    }

    /// Get next smaller frame size (for shrinking on errors)
    pub fn smaller(&self) -> Self {
        match self {
            FrameSize::Long => FrameSize::Standard,
            FrameSize::Standard => FrameSize::Medium,
            FrameSize::Medium => FrameSize::Short,
            FrameSize::Short => FrameSize::Short,
        }
    }

    /// Get next larger frame size (for growing on success)
    pub fn larger(&self) -> Self {
        match self {
            FrameSize::Short => FrameSize::Medium,
            FrameSize::Medium => FrameSize::Standard,
            FrameSize::Standard => FrameSize::Long,
            FrameSize::Long => FrameSize::Long,
        }
    }
    ...
    pub fn name(&self) -> &'static str {
        match self {
            ...
            FrameSize::Standard => "Standard",
            FrameSize::Long => "Long",
        }
    }
}

/// Sliding frame configuration computed from base FrameConfig and FrameSize
#[derive(Debug, Clone, Copy)]
pub struct SlidingFrameConfig {
    /// Base speed level
    pub level: u8,
    /// FFT size (unchanged from base)
    pub fft_size: usize,
    /// Adjusted number of STANAG symbols
    pub symbols: usize,
    /// Adjusted FEC block size in bits
    pub fec_block_bits: usize,
    /// Adjusted maximum payload bytes per frame
    pub max_payload_bytes: usize,
    /// Frame size setting
    pub frame_size: FrameSize,
    /// Modulation type (unchanged from base)
    pub modulation: ModulationType,
    /// Code rate (unchanged from base)
    pub code_rate: CodeRate,
}

impl SlidingFrameConfig {
    /// Create sliding frame config from base config and frame size
    pub fn from_base(base: &FrameConfig, frame_size: FrameSize) -> Self {
        let mult = frame_size.multiplier();

        // Scale symbols by multiplier, ensure minimum of 10 symbols
        let symbols = ((base.symbols as f32 * mult).round() as usize).max(10);

        // Scale fec_block_bits proportionally to symbols
        let fec_block_bits = ((base.fec_block_bits as f32 * mult).round() as usize).max(288);

        // Calculate max_payload_bytes from fec_block_bits
        // fec_block_bits is info bits, max_payload = (info_bits / 8) - 16 header overhead
        let max_payload_bytes = if fec_block_bits > 128 {
            (fec_block_bits - 128) / 8
        } else {
            0
        };
        ...
        Self {
            ...
            code_rate: base.code_rate,
        }
    }

    /// Get approximate frame duration in milliseconds
    pub fn frame_duration_ms(&self) -> u32 {
        let symbol_duration_ms = (self.fft_size as f32 / 48000.0) * 1000.0;
        (self.symbols as f32 * symbol_duration_ms).round() as u32
    }
}

/// Get sliding frame config for a speed level, bandwidth, and frame size
pub fn get_sliding_frame_config(level: u8, bandwidth: Bandwidth, frame_size: FrameSize) -> SlidingFrameConfig {
    let base = get_frame_config(level, bandwidth);
    SlidingFrameConfig::from_base(base, frame_size)
}
...
/// STANAG 39-tone DQPSK frame configurations for 2300 Hz bandwidth
/// 39 tones × 2 bits (DQPSK) = 78 bits/symbol
/// Symbol duration: 22.5 ms (1080 samples at 48 kHz)
/// fft_size = 853 (ACTIVE_SYMBOL_SAMPLES for demodulator FFT)
/// LDPC: Rate 1/2 k=972, Rate 2/3 k=1296, Rate 3/4 k=1458, Rate 5/6 k=1620
pub const FRAME_CONFIG_2300: [FrameConfig; 17] = [
    // Mode 1-4: FSK, rate 1/2 (robust fallback, unchanged)
    ...
    // STANAG modes 5-8: 25 symbols × 78 bits = 1950 raw bits, 1 LDPC codeword (1944)
    // Mode 5: rate 1/2, k=972
    FrameConfig { level: 5, fft_size: 853, symbols: 25, fec_block_bits: 972, max_payload_bytes: 105, frame_duration_ms: 562, modulation: ModulationType::Qpsk, code_rate: CodeRate::Rate1_2 },
    // Mode 6: rate 2/3, k=1296
    FrameConfig { level: 6, fft_size: 853, symbols: 25, fec_block_bits: 1296, max_payload_bytes: 146, frame_duration_ms: 562, modulation: ModulationType::Qpsk, code_rate: CodeRate::Rate2_3 },
    // Mode 7: rate 3/4, k=1458
    FrameConfig { level: 7, fft_size: 853, symbols: 25, fec_block_bits: 1458, max_payload_bytes: 166, frame_duration_ms: 562, modulation: ModulationType::Qpsk, code_rate: CodeRate::Rate3_4 },
    // Mode 8: rate 5/6, k=1620
    FrameConfig { level: 8, fft_size: 853, symbols: 25, fec_block_bits: 1620, max_payload_bytes: 186, frame_duration_ms: 562, modulation: ModulationType::Qpsk, code_rate: CodeRate::Rate5_6 },
    // STANAG modes 9-12: 50 symbols × 78 bits = 3900 raw bits, 2 LDPC codewords (3888)
    // Mode 9: rate 1/2, k=1944
    FrameConfig { level: 9, fft_size: 853, symbols: 50, fec_block_bits: 1944, max_payload_bytes: 227, frame_duration_ms: 1125, modulation: ModulationType::Qpsk, code_rate: CodeRate::Rate1_2 },
    // Mode 10: rate 2/3, k=2592
    FrameConfig { level: 10, fft_size: 853, symbols: 50, fec_block_bits: 2592, max_payload_bytes: 308, frame_duration_ms: 1125, modulation: ModulationType::Qpsk, code_rate: CodeRate::Rate2_3 },
    // Mode 11: rate 3/4, k=2916
    FrameConfig { level: 11, fft_size: 853, symbols: 50, fec_block_bits: 2916, max_payload_bytes: 348, frame_duration_ms: 1125, modulation: ModulationType::Qpsk, code_rate: CodeRate::Rate3_4 },
    // Mode 12: rate 5/6, k=3240
    FrameConfig { level: 12, fft_size: 853, symbols: 50, fec_block_bits: 3240, max_payload_bytes: 389, frame_duration_ms: 1125, modulation: ModulationType::Qpsk, code_rate: CodeRate::Rate5_6 },
    // STANAG modes 13-16: 75 symbols × 78 bits = 5850 raw bits, 3 LDPC codewords (5832)
    // Mode 13: rate 1/2, k=2916
    FrameConfig { level: 13, fft_size: 853, symbols: 75, fec_block_bits: 2916, max_payload_bytes: 348, frame_duration_ms: 1687, modulation: ModulationType::Qpsk, code_rate: CodeRate::Rate1_2 },
    // Mode 14: rate 2/3, k=3888
    FrameConfig { level: 14, fft_size: 853, symbols: 75, fec_block_bits: 3888, max_payload_bytes: 470, frame_duration_ms: 1687, modulation: ModulationType::Qpsk, code_rate: CodeRate::Rate2_3 },
    // Mode 15: rate 3/4, k=4374
    FrameConfig { level: 15, fft_size: 853, symbols: 75, fec_block_bits: 4374, max_payload_bytes: 530, frame_duration_ms: 1687, modulation: ModulationType::Qpsk, code_rate: CodeRate::Rate3_4 },
    // Mode 16: rate 5/6, k=4860
    FrameConfig { level: 16, fft_size: 853, symbols: 75, fec_block_bits: 4860, max_payload_bytes: 591, frame_duration_ms: 1687, modulation: ModulationType::Qpsk, code_rate: CodeRate::Rate5_6 },
    // Mode 17: Same as mode 16 (max for 2300Hz)
    FrameConfig { level: 17, fft_size: 853, symbols: 75, fec_block_bits: 4860, max_payload_bytes: 591, frame_duration_ms: 1687, modulation: ModulationType::Qpsk, code_rate: CodeRate::Rate5_6 },
];

/// STANAG 4197 frame configurations for 500 Hz bandwidth
/// 11 tones × 2 bits (DQPSK) = 22 bits/symbol
/// Symbol duration: 22.5 ms (1080 samples at 48 kHz)
/// fft_size = 853 (ACTIVE_SYMBOL_SAMPLES for demodulator FFT)
...
pub const FRAME_CONFIG_500: [FrameConfig; 17] = [
    ...
    FrameConfig { level: 4, fft_size: 512, symbols: 150, fec_block_bits: 288, max_payload_bytes: 20, frame_duration_ms: 1600, modulation: ModulationType::Fsk, code_rate: CodeRate::Rate1_2 },
    // STANAG modes 5-8: 90 symbols × 22 bits = 1980 raw bits, 1 LDPC codeword (1944)
    // frame_duration = 90 × 22.5 = 2025 ms
    // Mode 5: rate 1/2, k=972
    FrameConfig { level: 5, fft_size: 853, symbols: 90, fec_block_bits: 972, max_payload_bytes: 105, frame_duration_ms: 2025, modulation: ModulationType::Qpsk, code_rate: CodeRate::Rate1_2 },
    // Mode 6: rate 2/3, k=1296
    FrameConfig { level: 6, fft_size: 853, symbols: 90, fec_block_bits: 1296, max_payload_bytes: 146, frame_duration_ms: 2025, modulation: ModulationType::Qpsk, code_rate: CodeRate::Rate2_3 },
    // Mode 7: rate 3/4, k=1458
    FrameConfig { level: 7, fft_size: 853, symbols: 90, fec_block_bits: 1458, max_payload_bytes: 166, frame_duration_ms: 2025, modulation: ModulationType::Qpsk, code_rate: CodeRate::Rate3_4 },
    // Mode 8: rate 5/6, k=1620
    FrameConfig { level: 8, fft_size: 853, symbols: 90, fec_block_bits: 1620, max_payload_bytes: 186, frame_duration_ms: 2025, modulation: ModulationType::Qpsk, code_rate: CodeRate::Rate5_6 },
    // STANAG modes 9-12: 180 symbols × 22 bits = 3960 raw bits, 2 LDPC codewords (3888)
    // frame_duration = 180 × 22.5 = 4050 ms
    // Mode 9: rate 1/2, k=1944
    FrameConfig { level: 9, fft_size: 853, symbols: 180, fec_block_bits: 1944, max_payload_bytes: 227, frame_duration_ms: 4050, modulation: ModulationType::Qpsk, code_rate: CodeRate::Rate1_2 },
    // Mode 10: rate 2/3, k=2592
    FrameConfig { level: 10, fft_size: 853, symbols: 180, fec_block_bits: 2592, max_payload_bytes: 308, frame_duration_ms: 4050, modulation: ModulationType::Qpsk, code_rate: CodeRate::Rate2_3 },
    // Mode 11: rate 3/4, k=2916
    FrameConfig { level: 11, fft_size: 853, symbols: 180, fec_block_bits: 2916, max_payload_bytes: 348, frame_duration_ms: 4050, modulation: ModulationType::Qpsk, code_rate: CodeRate::Rate3_4 },
    // Mode 12: rate 5/6, k=3240
    FrameConfig { level: 12, fft_size: 853, symbols: 180, fec_block_bits: 3240, max_payload_bytes: 389, frame_duration_ms: 4050, modulation: ModulationType::Qpsk, code_rate: CodeRate::Rate5_6 },
    // Mode 13 (500 Hz max): 270 symbols × 22 bits = 5940 raw bits, 3 LDPC codewords (5832)
    // frame_duration = 270 × 22.5 = 6075 ms
    // rate 1/2, k=2916
    FrameConfig { level: 13, fft_size: 853, symbols: 270, fec_block_bits: 2916, max_payload_bytes: 348, frame_duration_ms: 6075, modulation: ModulationType::Qpsk, code_rate: CodeRate::Rate1_2 },
    // Modes 14-17: DO NOT EXIST for 500Hz - fallback entries (same as mode 13)
    FrameConfig { level: 14, fft_size: 853, symbols: 270, fec_block_bits: 2916, max_payload_bytes: 348, frame_duration_ms: 6075, modulation: ModulationType::Qpsk, code_rate: CodeRate::Rate1_2 },
    FrameConfig { level: 15, fft_size: 853, symbols: 270, fec_block_bits: 2916, max_payload_bytes: 348, frame_duration_ms: 6075, modulation: ModulationType::Qpsk, code_rate: CodeRate::Rate1_2 },
    FrameConfig { level: 16, fft_size: 853, symbols: 270, fec_block_bits: 2916, max_payload_bytes: 348, frame_duration_ms: 6075, modulation: ModulationType::Qpsk, code_rate: CodeRate::Rate1_2 },
    FrameConfig { level: 17, fft_size: 853, symbols: 270, fec_block_bits: 2916, max_payload_bytes: 348, frame_duration_ms: 6075, modulation: ModulationType::Qpsk, code_rate: CodeRate::Rate1_2 },
];

/// STANAG 4197 frame configurations for 2750 Hz bandwidth
/// 39 tones × 2 bits (DQPSK) = 78 bits/symbol
/// Symbol duration: 22.5 ms (1080 samples at 48 kHz)
/// fft_size = 853 (ACTIVE_SYMBOL_SAMPLES for demodulator FFT)
/// LDPC: Rate 1/2 k=972, Rate 2/3 k=1296, Rate 3/4 k=1458, Rate 5/6 k=1620
pub const FRAME_CONFIG_2750: [FrameConfig; 17] = [
    ...
    // Mode 16: rate 5/6, k=4860
    FrameConfig { level: 16, fft_size: 853, symbols: 75, fec_block_bits: 4860, max_payload_bytes: 591, frame_duration_ms: 1687, modulation: ModulationType::Qpsk, code_rate: CodeRate::Rate5_6 },
    // Mode 17: 100 symbols × 78 bits = 7800 raw bits, 4 LDPC codewords (7776)
    // rate 5/6, k=6480
    FrameConfig { level: 17, fft_size: 853, symbols: 100, fec_block_bits: 6480, max_payload_bytes: 794, frame_duration_ms: 2250, modulation: ModulationType::Qpsk, code_rate: CodeRate::Rate5_6 },
];
