...
namespace ModeCapabilities {
    ...
    // Optional CONNECT capability bit for negotiated runtime waveform changes
    // via MODE_CHANGE payload extension.
    constexpr uint8_t MODE_CHANGE_WAVEFORM = 0x40;
    constexpr uint8_t ALL        = OFDM_COX | OTFS_EQ | OTFS_RAW | MFSK | MC_DPSK | OFDM_CHIRP;
    constexpr uint8_t WAVEFORM_MASK = ALL;

    // Optional CONNECT capability bit (independent of waveform selection).
    // When set by both peers and negotiated waveform is MC_DPSK, peers may
    // enable per-codeword channel interleaving for MC-DPSK DATA frames.
    constexpr uint8_t MC_DPSK_CHANNEL_INTERLEAVE = 0x80;
}
...
// CONNECT_ACK feature packing:
// measured_snr byte keeps its normal SNR meaning, with:
//   bit0 = MC-DPSK DATA-channel interleave support
//   bit1 = runtime MODE_CHANGE waveform-switch support
// This reduces SNR granularity by two LSBs (0.5 dB) for CONNECT_ACK only.
namespace ConnectAckFeatures {
    constexpr uint8_t MC_DPSK_CHANNEL_INTERLEAVE = 0x01;
    constexpr uint8_t MODE_CHANGE_WAVEFORM = 0x02;
}

inline uint8_t encodeConnectAckMeasuredSNR(float snr_db,
                                           bool mc_dpsk_channel_interleave,
                                           bool mode_change_waveform) {
    uint8_t enc = encodeSNR(snr_db) &
                  static_cast<uint8_t>(~(ConnectAckFeatures::MC_DPSK_CHANNEL_INTERLEAVE |
                                         ConnectAckFeatures::MODE_CHANGE_WAVEFORM));
    if (mc_dpsk_channel_interleave) {
        enc |= ConnectAckFeatures::MC_DPSK_CHANNEL_INTERLEAVE;
    }
    if (mode_change_waveform) {
        enc |= ConnectAckFeatures::MODE_CHANGE_WAVEFORM;
    }
    return enc;
}

inline float decodeConnectAckSNR(uint8_t encoded) {
    uint8_t snr_only = encoded & static_cast<uint8_t>(
        ~(ConnectAckFeatures::MC_DPSK_CHANNEL_INTERLEAVE |
          ConnectAckFeatures::MODE_CHANGE_WAVEFORM));
    return decodeSNR(snr_only);
}

inline bool decodeConnectAckMCDPSKChannelInterleave(uint8_t encoded) {
    return (encoded & ConnectAckFeatures::MC_DPSK_CHANNEL_INTERLEAVE) != 0;
}

inline bool decodeConnectAckModeChangeWaveform(uint8_t encoded) {
    return (encoded & ConnectAckFeatures::MODE_CHANGE_WAVEFORM) != 0;
}

namespace ModeChangePayload {
    constexpr uint8_t HAS_WAVEFORM = 0x80;
    constexpr uint8_t WAVEFORM_MASK = 0x7F;

    inline bool isRuntimeWaveform(WaveformMode mode) {
        switch (mode) {
    ...
    inline uint8_t encodeWaveform(std::optional<WaveformMode> waveform) {
        if (!waveform.has_value() || !isRuntimeWaveform(*waveform)) {
            return 0;
        }
        return HAS_WAVEFORM |
               (static_cast<uint8_t>(*waveform) & WAVEFORM_MASK);
    }

    inline std::optional<WaveformMode> decodeWaveform(uint8_t encoded) {
        if ((encoded & HAS_WAVEFORM) == 0) {
            return std::nullopt;
        }
        WaveformMode mode = static_cast<WaveformMode>(encoded & WAVEFORM_MASK);
        if (!isRuntimeWaveform(mode)) {
            return std::nullopt;
        }
        return mode;
    }
...
struct ControlFrame {
    ...
    struct ModeChangeInfo {
        ...
        float fading_index;
        uint8_t reason;
        bool waveform_change_requested;
    ...
    ModeChangeInfo getModeChangeInfo() const {
        ...
        auto waveform = ModeChangePayload::decodeWaveform(payload[5]);
        info.waveform_change_requested = waveform.has_value();
        info.waveform_mode = waveform.value_or(WaveformMode::AUTO);
        return info;
    }
};
...
//                         CONNECT_ACK = responder's measured SNR + feature bits
//
...
struct ConnectFrame {
    ...
    static ConnectFrame makeConnectAck(const std::string& src, const std::string& dst,
                                        ...
                                        bool mc_dpsk_channel_interleave = false,
                                        bool mode_change_waveform = false);
