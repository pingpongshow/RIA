...
// Defensive cap for received file metadata to avoid pathological allocations
// from malformed payloads.
constexpr uint32_t MAX_RX_FILE_SIZE_BYTES = 16U * 1024U * 1024U;
...
bool FileTransferController::processFileStart(const Bytes& payload) {
    ...
    if (rx_filename_.empty()) {
        rx_filename_ = "received.bin";
    }
    ...
    if (rx_expected_size_ == 0 || rx_expected_size_ > MAX_RX_FILE_SIZE_BYTES) {
        LOG_MODEM(WARN, "FileTransfer: Ignoring invalid FILE_START size=%u bytes", rx_expected_size_);
        resetRxState();
        return true;
    }
    ...
    try {
        ...
        LOG_MODEM(ERROR, "FileTransfer: reserve() failed for incoming file (%u bytes)",
                  rx_expected_size_);
        if (on_received_) {
            on_received_("", false);
        }
