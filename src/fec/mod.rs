...
impl CodeRate {
    ...
    /// LDPC uses block codes (no tail bits like Turbo codes)
    /// Returns the encoded bits based on code rate
    pub fn ldpc_output_bits(&self, fec_block_bits: usize) -> usize {
        // LDPC is a block code - no tail bits needed
        self.output_bits(fec_block_bits)
    }
